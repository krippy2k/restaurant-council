export interface AlsoConsideredRestaurant {
  id: string;
  name: string;
  cuisines: string[];
...
export interface AlsoConsideredContext {
  restaurant: AlsoConsideredRestaurant;
  evaluations: Array<{
...
export function whyNotRecommended(input: AlsoConsideredContext): string[] {
  ...
  if (cuisinePrefers.length && reasons.length < 2) {
    const wanted = cuisinePrefers.flatMap((item) =>
      asStringArray(item.value).map((value) => value.toLowerCase())
    );
